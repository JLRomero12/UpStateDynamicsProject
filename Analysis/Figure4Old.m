...
cd('C:\Users\BuonoLab\Desktop\Juan Luis\MainStudy\SimultaneousRecording')
load META
...
%% INIT GRAPHICS
PVFigure = figure('Name','MainFigure');
set(gcf,'color','w');
ScreenSize = get(groot,'ScreenSize');
PVFigure.Position = [0 0 ScreenSize(3)/1.2 ScreenSize(4)/1.2]; %This is to Force the figure to have a vertical orientation
...
xlim([0.22 0.4])
ylim([-66 -36])
PVFigure.Children(1).YTick = [];
PVFigure.Children(1).XTick = [];
...
title('Difference UpStart','FontSize',14)
ylim([-90 250])
box off

if h==1
    hold on
    text(0.85,223,'p<0.05','FontWeight','bold','FontSize',12)
end
