function [eventtimes, offtimes, Tabove, areaabove, numThrX, upstate, updur, upstart, upend] ...
    =MAKESEGMENTS(trace,thr,MaxIntEvent,MaxIntUpstate,SegLen,MinAboveTime,MinOnOffInt,preEventTime,KurtosisFlag)
    ...
    if ( start>preEventTime && start+SegLen<length(trace) )
        ...
        if (reject==0)
            ...
            if (1)
                ...
                line([preEventTime preEventTime],[min(trace) max(trace)],'color','k','linestyle',':')
                str=sprintf('i=%4d, j=%4d, start/stop=%4d/%4d',i,j,seg(1),seg(length(seg)));
                title(str)
                drawnow;
                waitforbuttonpress
            end
